...
class Oli_bot(sc2.BotAI):
    async def on_step(self, iteration):
        ...
        cc = (self.structures(COMMANDCENTER) | self.structures(ORBITALCOMMAND))
        cc = cc.first
        ...
        if self.units(SCV).amount < 18 and self.structures(SUPPLYDEPOT).exists and self.can_afford(SCV) and self.structures(BARRACKS).ready.amount < 1 and cc.noqueue:
                await self.do(cc.train(SCV))
        ...
        if self.structures(REFINERY).amount < 1 and self.already_pending(BARRACKS) > 0:
            if self.can_afford(REFINERY):
                vgs = self.state.vespene_geyser.closer_than(20.0, cc)
                for vg in vgs:
                    if self.structures(REFINERY).closer_than(1.0, vg).exists:
                        break
        ...
        if self.units(BARRACKS).exists:
            for brks_react in self.units(BARRACKS).ready:
                await self.do(brks_react.build(BARRACKSREACTOR))
        
                
        if self.units(BARRACKSREACTOR).exists:
            for brks_react2 in self.units(BARRACKS).ready:
        ...
        if self.units(BARRACKSTECHLAB).ready.exists and self.units(BARRACKS).amount < 2:
            for react in self.units(BARRACKS).ready:
        ...
        if self.units(BARRACKS).ready.exists and not self.already_pending(FACTORY):
            f = self.units(FACTORY)
            if not f.exists:
                if self.can_afford(FACTORY):
                    await self.build(FACTORY, near=cc.position.towards(self.game_info.map_center, 3))
            elif f.ready.exists and self.units(STARPORT).amount < 2:
                if self.can_afford(STARPORT):
                    await self.build(STARPORT, near=cc.position.towards(self.game_info.map_center, 30).random_on_distance(3))
